package com.impetus.code.examples.hadoop.cassandra.wordcount;
...
public class WordCountSetup
{
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        mutationMap = getMutationMap(ByteBufferUtil.bytes("key0"), WordCount.INPUT_COLUMN_FAMILY, c);
        ...
        for (int i=0; i<1000; i++)
        {
            c = new Column()
                ...
                .setValue(ByteBufferUtil.bytes("word1"))
                .setTimestamp(System.currentTimeMillis());
            addToMutationMap(mutationMap, ByteBufferUtil.bytes("key" + i), WordCount.INPUT_COLUMN_FAMILY, c);
        }
        client.batch_mutate(mutationMap, ConsistencyLevel.ONE);
    ...
    private static void setupKeyspace(Cassandra.Iface client) throws TException, InvalidRequestException, SchemaDisagreementException {
        ...
        CfDef input = new CfDef(WordCount.KEYSPACE, WordCount.INPUT_COLUMN_FAMILY);
